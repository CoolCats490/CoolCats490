...
const GrouptListItem = (props) => {
  ...
  return (
    <Container className="pb-0 pt-1 no-underline" key={props.id}>
      <Link to={{ pathname: `/groups/${props.id}` }} className="text-decoration-none text-dark">
          
          <Row>
          <Col className="w-50" md="6" >
            <Image src={props.groupPic||defaultPic} alt="group image"  rounded className="img-fluid img-max"/>
          </Col>
          <Col className="mx-4">
          <h5 className="mt-4">
            <strong className="text-info">{props.title}</strong>
              ...
              <Badge className="mb-2 px-2">Memebers in group:<Badge className="bg-dark">
                <strong>{props.members.length}</strong></Badge>
